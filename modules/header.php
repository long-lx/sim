...
$qp=mysql_query("SELECT id, ptitle FROM page WHERE pcode='menu' order by id asc");

$nav='<li><a href="'.$myinfo[my_domain].'">Trang chủ</a></li>';
while ($rqp=mysql_fetch_array($qp))
{
	$nav.='<li><a href="'.$myinfo[my_domain].'/v'.$rqp['id'].xoadau($rqp['ptitle']).'.html" id='.$rqp['id'].'>'.$rqp['ptitle'].'</a></li>';
}
$stv->assign("nav",$nav);
/*
...
elseif ($_GET['sosim'])
{
	...
	$title=$_GET['sosim']." &laquo; Sim Số Đẹp &laquo; ".$my_title." &laquo; nganhangsimsodep.com";
}
elseif ($_GET['link'])
{
...
{
	$title="Sim số đẹp Viettel &laquo; Sim số đẹp Mobifone &laquo; Sim số đẹp Vinaphone &laquo; Ngân Hàng Sim số đẹp giá rẻ !";
}
